...
export async function getGlosas(supabase, clinicaId) {
  ...
  const { data: lotes } = await supabase
    .from('lote')
    .select('id, operadora, competencia')
    .eq('clinica_id', clinicaId);

  if (!lotes?.length) return [];

  const operadoraPorLote = new Map(lotes.map((l) => [l.id, l.operadora]));
  const competenciaPorLote = new Map(lotes.map((l) => [l.id, l.competencia]));
  ...
  return (itens ?? [])
    .map((i) => {
      ...
      return {
        ...
        paciente: g?.beneficiario ?? '—',
        operadora: operadoraPorLote.get(g?.lote_id) || '—',
        /* Competência do demonstrativo de origem. Serve para recortar o
           relatório por mês sem ter que refazer o caminho lote → guia →
           item em quem consome. */
        competencia: competenciaPorLote.get(g?.lote_id) ?? null,
...
/* Só a contagem de itens glosados, para o badge da sidebar.

   Existe separado de getGlosas() porque o shell do app precisa do número
   em toda tela, e trazer a lista inteira só para chamar .length seria
   pagar três consultas e o mapeamento completo por navegação.
   guia e item não têm clinica_id — o vínculo é lote → guia → item. */
export async function getGlosasCount(supabase, clinicaId) {
...
export async function getRecursos(supabase, clinicaId) {
  ...
  const { data: recursos } = await supabase
    .from('recurso')
    .select('id, guia_id, valor_pleiteado, valor_recuperado, status, criado_em, enviado_em, resolvido_em')
    .in('guia_id', guias.map((g) => g.id))
    .order('criado_em', { ascending: false });
  ...
  return (recursos ?? []).map((r) => {
    ...
    return {
      ...
      guia: g?.numero_guia ?? '—',
      operadora: operadoraPorLote.get(g?.lote_id) || '—',
      /* Mesma razão de getGlosas: permite recortar por mês sem refazer o
         caminho até o lote. */
      competencia: competenciaPorLote.get(g?.lote_id) ?? null,
      gerado: formatarDataHora(r.criado_em),
      valor: num(r.valor_pleiteado),
      /* null quando o recurso não chegou a 'ganho'. Em recurso 'ganho'
         gravado antes da coluna existir, null significa aceite integral
         — quem soma trata o fallback. */
      valorRecuperado: r.valor_recuperado == null ? null : num(r.valor_recuperado),
      status: r.status,
      enviadoEm: r.enviado_em ? formatarDataHora(r.enviado_em) : null,
      resolvidoEm: r.resolvido_em ? formatarDataHora(r.resolvido_em) : null,
      /* Cru também, porque os relatórios contam dias entre as duas —
         a versão formatada é para exibir, não para calcular. */
      enviadoEmISO: r.enviado_em ?? null,
      resolvidoEmISO: r.resolvido_em ?? null,
    };
  });
}
...
export async function getRecurso(supabase, clinicaId, recursoId, { planoPago }) {
  ...
  const { data: recurso } = await supabase
    .from('recurso')
    .select('id, guia_id, texto_gerado, valor_pleiteado, valor_recuperado, status, criado_em, enviado_em, resolvido_em')
    .eq('id', recursoId)
    .maybeSingle();
  ...
  return {
    ...
    valorRecuperado:
      recurso.valor_recuperado == null ? null : num(recurso.valor_recuperado),
    status: recurso.status,
    gerado: formatarDataHora(recurso.criado_em),
    /* null enquanto a transição não aconteceu — a tela usa a ausência
       para não escrever "enviado em —". */
    enviadoEm: recurso.enviado_em ? formatarDataHora(recurso.enviado_em) : null,
    resolvidoEm: recurso.resolvido_em ? formatarDataHora(recurso.resolvido_em) : null,
    texto,
    completo: planoPago,
...
export function calcularResumo({ lotes = [], glosas = [], recursos = [] }) {
  ...
  /* A operadora costuma reverter só parte da glosa: pleiteou R$ 300,
     pagou R$ 200. Somar o pleiteado reportaria dinheiro que não entrou
     no caixa. `valorRecuperado` nulo cai no pleiteado — é o caso do
     recurso marcado como ganho antes da coluna existir, quando aceite
     integral era a única possibilidade. */
  const recuperado = recursos
    .filter((r) => r.status === 'ganho')
    .reduce((s, r) => s + (r.valorRecuperado ?? r.valor), 0);
