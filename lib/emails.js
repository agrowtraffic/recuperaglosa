...
const TEMPLATE_CONFIRMAR_CONTA = 'recupera-glosa-confirmacao-conta';
const TEMPLATE_REDEFINIR_SENHA = 'recupera-glosa-redefinir-senha';
...
/* ------------------------------------------------------------
   CONFIRMAÇÃO DE CONTA — link oficial gerado pelo Supabase e
   entregue pelo Resend. Evita o fallback inglês quando o endpoint
   de templates hospedados do Supabase estiver indisponível.
   ------------------------------------------------------------ */
export async function enviarConfirmacaoConta({ para, confirmationUrl, userId }) {
  if (!para) return { enviado: false, motivo: 'usuário sem e-mail' };
  if (!confirmationUrl) return { enviado: false, motivo: 'confirmação sem URL' };

  const assunto = 'Confirme sua conta na Recupera Glosa';
  ...
  const { error } = await resend.emails.send(
    ...
    {
      headers: {
        'Idempotency-Key': `confirmar-conta-${userId}`,
      },
    }
  );
...
/* ------------------------------------------------------------
   RECUPERAÇÃO DE SENHA — link oficial gerado pelo Supabase e
   entregue pelo Resend. Evita o fallback inglês do template nativo.
   ------------------------------------------------------------ */
export async function enviarRedefinicaoSenha({ para, resetUrl, requestId }) {
  if (!para) return { enviado: false, motivo: 'usuário sem e-mail' };
  if (!resetUrl) return { enviado: false, motivo: 'redefinição sem URL' };

  const assunto = 'Redefina sua senha da Recupera Glosa';
  ...
  const { error } = await resend.emails.send(
    ...
    {
      headers: {
        'Idempotency-Key': `redefinir-senha-${requestId}`,
      },
    }
  );
