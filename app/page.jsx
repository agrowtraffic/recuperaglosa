...
function Lotes({query,setQuery,dashboardData}){
 ...
 const lotesData=dashboardData?.lotes||lotes;
...
function Guias(){
 const [query,setQuery]=useState('');
 const [operadora,setOperadora]=useState('Todas');
 const filtered=useMemo(()=>guias.filter(r=>r.join(' ').toLowerCase().includes(query.toLowerCase())&&(operadora==='Todas'||r[2]===operadora)),[query,operadora]);
 return <><Toolbar query={query} setQuery={setQuery} placeholder="Buscar guia, paciente ou operadora..."><SelectFilter value={operadora} onChange={setOperadora} options={['Todas','Unimed','Amil','SulAmérica','Bradesco']} label="Operadora"/></Toolbar><div className="metric-row"><Metric label="Guias auditadas" value="342"/><Metric label="Com glosa" value="42" warn/><Metric label="Sem glosa" value="300" positive/><Metric label="Taxa de aprovação" value="87,7%" positive/></div><div className="page-card"><div className="card-title"><div><h2>Guias auditadas</h2><p>Valores apresentados, pagos e glosados</p></div><button className="outline compact"><Icon name="download"/>Exportar</button></div><DataTable heads={['Nº da guia','Paciente','Operadora','Tipo','Apresentado','Pago','Glosado','']} rows={filtered.map(r=>[r[0],r[1],r[2],r[3],r[4],r[5],<b className="orange-text" key="g">{r[6]}</b>,<button className="row-action" aria-label={`Ver guia ${r[0]}`} key="a"><Icon name="eye" size={17}/></button>])}/></div></>}

function Glosas(){
 const [query,setQuery]=useState('');
 const [prioridade,setPrioridade]=useState('Todas');
 const filtered=useMemo(()=>glosas.filter(r=>r.join(' ').toLowerCase().includes(query.toLowerCase())&&(prioridade==='Todas'||r[4]===prioridade)),[query,prioridade]);
 return <><Toolbar query={query} setQuery={setQuery} placeholder="Buscar guia, código ou motivo..."><SelectFilter value={prioridade} onChange={setPrioridade} options={['Todas','Alta','Média','Baixa']} label="Prioridade"/></Toolbar><div className="metric-row"><Metric label="Total glosado" value="R$ 18.430,75" warn/><Metric label="Recorrível" value="R$ 15.285,40" positive/><Metric label="Em análise" value="R$ 2.045,35"/><Metric label="Não recorrível" value="R$ 1.100,00"/></div><div className="split-layout"><div className="page-card"><div className="card-title"><div><h2>Glosas identificadas</h2><p>Ordenadas por potencial de recuperação</p></div></div><DataTable heads={['Guia','Código','Motivo','Valor','Prioridade','Situação']} rows={filtered.map(r=>[r[0],r[1],r[2],<b className="orange-text" key="v">{r[3]}</b>,<Priority key="p" text={r[4]}/>,<Status key="s" text={r[5]}/>])}/></div><aside className="insight-card"><span className="insight-icon"><Icon name="dollar"/></span><h3>Maior oportunidade</h3><strong>R$ 6.450,20</strong><p>35% do valor recuperável vem de procedimentos classificados como “não cobertos”.</p><button className="primary full">Criar recursos</button></aside></div></>}

function Recursos(){
 const [query,setQuery]=useState('');
 const [status,setStatus]=useState('Todos');
 const filtered=useMemo(()=>recursos.filter(r=>r.join(' ').toLowerCase().includes(query.toLowerCase())&&(status==='Todos'||r[4]===status)),[query,status]);
 const columns=['Rascunho','Pronto','Enviado','Aceito'];
 return <><Toolbar query={query} setQuery={setQuery} placeholder="Buscar recurso, guia ou motivo..."><SelectFilter value={status} onChange={setStatus} options={['Todos','Rascunho','Pronto','Enviado','Aceito']} label="Status"/></Toolbar><div className="metric-row"><Metric label="Recursos prontos" value="18" positive/><Metric label="Enviados" value="12"/><Metric label="Aceitos" value="8" positive/><Metric label="Taxa de sucesso" value="66,7%" positive/></div>
 <div className="kanban" aria-label="Pipeline de recursos">{columns.map(column=><section className="kanban-column" key={column}><header><span>{column}</span><b>{recursos.filter(r=>r[4]===column).length}</b></header>{recursos.filter(r=>r[4]===column&&(status==='Todos'||r[4]===status)&&r.join(' ').toLowerCase().includes(query.toLowerCase())).map(r=><article className="kanban-card" key={r[0]}><div><strong>{r[0]}</strong><Status text={r[4]}/></div><p>{r[1]}</p><span>{r[2]}</span><footer><b>{r[3]}</b><button className="row-action" aria-label={`Ver ${r[0]}`}><Icon name="eye" size={16}/></button></footer></article>)}{!recursos.some(r=>r[4]===column)&&<div className="kanban-empty">Nenhum recurso nesta etapa</div>}</section>)}</div>
 <div className="page-card resources-table"><div className="card-title"><div><h2>Todos os recursos</h2><p>Textos gerados automaticamente para contestação</p></div><button className="outline compact"><Icon name="download"/>Baixar todos</button></div><DataTable heads={['Recurso','Referência','Motivo','Valor','Status','Ações']} rows={filtered.map(r=>[r[0],r[1],r[2],<b className="money" key="m">{r[3]}</b>,<Status key="s" text={r[4]}/>,<div className="action-pair" key="a"><button className="row-action" aria-label="Visualizar"><Icon name="eye" size={17}/></button><button className="row-action" aria-label="Copiar"><Icon name="copy" size={17}/></button><button className="row-action" aria-label="Baixar"><Icon name="download" size={17}/></button></div>])}/></div>
 <div className="resource-preview"><div><span className="eyebrow">PRÉVIA DO RECURSO</span><h3>Contestação — código 1019</h3><p>Prezados, solicitamos a reanálise da glosa aplicada à guia nº 00038492. O procedimento foi realizado conforme indicação clínica e encontra-se coberto pelas condições contratuais vigentes...</p></div><button className="primary"><Icon name="copy"/>Copiar texto</button></div></>}

function Relatorios(){
 const [periodo,setPeriodo]=useState('30 dias');
 return <><div className="report-head"><div className="segmented">{['30 dias','3 meses','12 meses'].map(p=><button key={p} className={periodo===p?'active':''} onClick={()=>setPeriodo(p)}>Últimos {p}</button>)}</div><button className="outline compact"><Icon name="download"/>Exportar PDF</button></div><div className="metric-row"><Metric label="Valor apresentado" value="R$ 98.765,40"/><Metric label="Valor pago" value="R$ 80.334,65"/><Metric label="Valor glosado" value="R$ 18.430,75" warn/><Metric label="Recuperado" value="R$ 9.820,30" positive/></div><div className="report-grid"><div className="page-card chart-card"><div className="card-title"><div><h2>Evolução do valor recuperável</h2><p>Comparativo mensal — {periodo}</p></div></div><BarChart/></div><div className="page-card"><div className="card-title"><div><h2>Motivos de glosa</h2><p>Participação no valor total</p></div></div><div className="large-donut"><Donut data={motivosGlosa} large/><div>{motivosGlosa.map(item=><Legend key={item.name} color={item.color} t={item.name} v={`${item.value}%`}/>)}</div></div></div></div></>}
